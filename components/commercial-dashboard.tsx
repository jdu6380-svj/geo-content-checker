          ...
          <div className="evidra-home-grid">
            <div className="evidra-home-primary">
              <section className="evidra-upload-card" id="review" aria-labelledby="evidra-upload-title">
                ...
                {inputMode === "upload" ? <div className={`evidra-dropzone ${isDraggingFile ? "is-dragging" : ""}`} onDragOver={handleFileDragOver} onDragLeave={handleFileDragLeave} onDrop={handleFileDrop}><UploadCloud aria-hidden="true" /><h2 id="evidra-upload-title">拖入文章文件，或选择文件上传</h2><p>支持 Markdown、TXT 文本格式</p><label className="evidra-primary-button" htmlFor="evidra-file-input">选择文件 <ChevronRight aria-hidden="true" /></label><input id="evidra-file-input" type="file" accept=".md,.markdown,.txt,text/markdown,text/plain" onChange={(event) => void handleFilePick(event.target.files?.[0])} /><small>PDF、DOCX 请先复制正文到“粘贴正文”</small>{fileMessage ? <p className="evidra-file-message" role="status">{fileMessage}</p> : null}</div> : null}
                {inputMode === "paste" ? (
