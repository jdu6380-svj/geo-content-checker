...
export function ReportDimensionLedger({ report }: ReportDimensionLedgerProps) {
  return (
    <section className="report-dimension-ledger border-t border-[#e3e7eb]" aria-labelledby="dimension-ledger-heading">
      <div className="flex flex-wrap items-end justify-between gap-3 px-5 py-4 sm:px-6">
        <div>
          <p className="section-kicker">SCORE LEDGER</p>
          <h2 id="dimension-ledger-heading" className="mt-1.5 text-base font-semibold text-[#1f2933]">四维评分账本</h2>
          <p className="mt-1 text-xs leading-5 text-[#737d89]">固定权重与模型给出的评分依据逐项对应，总分不代表平台收录或排名。</p>
        </div>
        <span className="status-badge status-neutral px-2.5 py-1 text-[11px] font-semibold">权重合计 100</span>
      </div>

      <div className="report-dimension-table border-t border-[#e7eaed]">
            ...
            return (
              <li key={key} className="report-dimension-row px-5 py-4 sm:px-6">
                <div className="flex min-w-0 items-start gap-3">
                  ...
                  <span className="grid size-8 shrink-0 place-items-center rounded-md border border-[#dde2e6] bg-[#f8f9fa] text-[#59636f]">
                    <Icon aria-hidden="true" className="size-4" />
                  </span>
                  <span className="min-w-0">
                    <span className="block text-sm font-semibold text-[#252f38]">{label}</span>
                    <span className="mt-1 block text-[11px] leading-5 text-[#858c97]">{description}</span>
                  </span>
                </div>
                <div className="mt-3 flex items-baseline gap-1 md:mt-0">
                  <strong className="text-lg tabular-nums text-[#1f2933]">{dimension.score}</strong>
                  <span className="text-xs text-[#8b939e]">/ {dimension.max}</span>
                </div>
                <div className="mt-3 md:mt-0">
                  <div className="flex items-center justify-between gap-3 text-[10px] font-semibold text-[#737d89]">
                ...
                <p className="mt-3 text-sm leading-6 text-[#59636f] md:mt-0">{dimension.reason}</p>
              </li>
            );
          })}
        </ol>
